...
from library.tensorflow_utils import export_keras_to_tensorflow, \
    export_text_model_to_csv
from library.tokenizer_utils import word_tokenize

class training_processing(object):
    ...
    def get_architecture_file_path(model_dir_path):
        return model_dir_path + '/' + training_processing.model_name + '_architecture.json'

    @staticmethod
    def get_weight_file_path(model_dir_path):
        return model_dir_path + '/' + training_processing.model_name + '_weights.h5'

    @staticmethod
    def get_config_file_path(model_dir_path):
        return model_dir_path + '/' + training_processing.model_name + '_config.npy'
    ...
    def fit(self, text_data_model, text_label_pairs, model_dir_path, batch_size=None, epochs=None,
        ...
        np.save(model_dir_path + '/' + training_processing.model_name + '-history.npy', history.history)
    ...
    def export_tensorflow_model(self, output_fld):
        export_keras_to_tensorflow(self.model, output_fld, output_model_file=training_processing.model_name + '.pb')
        export_text_model_to_csv(self.config, output_fld, output_model_file=training_processing.model_name + '.csv')

def main():
    app = training_processing()
    app.test_run('minh khong nghe duoc')
